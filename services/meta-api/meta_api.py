...
async def lifespan(app: FastAPI):
    ...
    pool = await asyncpg.create_pool(
        ...
        server_settings={"search_path": pg_schema},
    )
    logger.info("连接池已创建 (max=%s, search_path=%s): %s", os.getenv("PG_POOL_MAX", "20"), pg_schema, PG_DSN.split("@")[-1])
    yield
    await pool.close()
...
INSERT INTO track_project (name, is_auto_create, status, create_time)
...
SELECT id, project_id, name FROM track_event_define
...
SELECT id, project_id, name FROM track_property_define
WHERE (project_id, name) IN (SELECT unnest($1::int[]), unnest($2::text[]));
"""

# 批量 upsert 事件-属性关联
SQL_BATCH_UPSERT_EVENT_PROPERTY = """
INSERT INTO track_event_property (event_id, property_id, update_time)
...
INSERT INTO track_user_id_mapping (project_name, distinct_id, map_id, map_type, login_id, load_time)
