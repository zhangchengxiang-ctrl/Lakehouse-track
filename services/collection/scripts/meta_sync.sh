...
sync_once() {
    # 1) 项目列表
    {
        ...
        psql $PG_CONN -t -A -F',' -c \
            "SELECT name, is_auto_create FROM track_project WHERE status = 1" 2>/dev/null || true
    ...
    # 2) 有效事件（项目+事件+accepted）
    {
        ...
        psql $PG_CONN -t -A -F',' -c \
            "SELECT p.name, e.name, e.accepted
             FROM track_event_define e
             JOIN track_project p ON e.project_id = p.id
    ...
    # 3) 有效属性（项目+属性+数据类型）
    {
        ...
        psql $PG_CONN -t -A -F',' -c \
            "SELECT p.name, pd.name, pd.data_type
             FROM track_property_define pd
             JOIN track_project p ON pd.project_id = p.id
