...
> **升级提示（重要）**：从 Paimon 0.8.x 升级到 0.9.x 时，建议清理 MinIO 中旧的 `paimon_data`（或直接执行 `make reset`）后再重建，避免残留元数据导致 Catalog/表不可见。

### 2.3 目录结构
```text
.
├── services/           # [容器服务配置] 各组件 Dockerfile、配置文件
│   ├── collection/     #   采集节点 (Nginx + Vector + MetaSync)
...
### 3.1 首次启动步骤
1. **安装依赖**：`make install`（含 Hive Metastore 客户端依赖）
2. **启动容器**：`docker compose up -d --build`
3. **初始化存储**：访问 [MinIO](http://localhost:9001) (minioadmin/minioadmin) 创建 bucket `Lakehouse`。
4. **提交任务**：`make run-sql`（宿主机直连 HMS 时：`HMS_URI=thrift://127.0.0.1:9083 make run-sql`）
...
1. **修改 PostgreSQL 数据**：
   ...
   "SET search_path TO StellarTrace; INSERT INTO cdc_test_orders (order_id, user_id, amount) VALUES ('o3', 'u3', 299.00);"
   ```
...
3. **验证更新同步**：
   ```bash
   docker compose exec -T postgres psql -U postgres -d postgres -c \
   "SET search_path TO StellarTrace; UPDATE cdc_test_orders SET amount = 300.00 WHERE order_id = 'o3';"
   ```
...
| **StarRocks Catalog 已存在** | 脚本已修复 DROP 顺序。若仍报错，可执行 `make run-sql ARGS=starrocks` 强制重刷。 |
| **Paimon 查询报错 (InvalidClassException)** | 典型原因是 **Flink/Paimon/StarRocks** 三端 JAR 版本不一致导致的类冲突。当前推荐版本线为：Flink **1.20.3** + Paimon **1.3.1** + StarRocks **3.5.12**（并确保 StarRocks 外部 Catalog 所需 JAR 由 `make install` 下载）。 |
| **Vector 无法写入 MinIO** | 检查 `Lakehouse` bucket 是否已创建。创建后需 `docker compose restart vector`。 |
| **Flink 任务 RESTARTING** | 通常是 S3 连接超时或 JAR 缺失。执行 `make fix` 自动重置任务。 |
...
- **PostgreSQL**: `127.0.0.1:5432` (postgres/postgres，默认库 postgres，业务 schema StellarTrace)
